...
public class FilteringExample {
	...
	public static void main(String[] args) {
		...
		// [2] List를 스트림으로 만들어서 반복(스트림을 통해 출력)
		developerList.stream().forEach(m -> System.out.println(m.getName()));
		System.out.println();

		// [1]과 [2]를 한번에 처리
		list.stream().filter(member -> member.getJob().equals("개발자")).collect(Collectors.toList())
